import './styles/About.css'
import sofiaImage from '../assets/sofia_placeholder.png'

function About() {
  return (
    <section id="about" className="about-section">
      <div className="about-container">
        <div className="about-image-wrapper">
          <img src={sofiaImage} alt="Sofia Knuts Johansson i sin salong" className="about-image" />
          <div className="about-image-decoration"></div>
        </div>
        <div className="about-content">
          <h2 className="about-heading">Om Mig</h2>
          <h3 className="about-subheading">Hälsosam ansikts- och kroppsvård för alla, helt utan tveksamma kemikalier.</h3>
          <p className="about-text">
            Jag brinner för att hjälpa dig hitta balans i vardagen genom en helhetssyn på hälsa. På Kornets Hälso Spa kombinerar jag fysiska behandlingar som massage och skönhetsvård med mental återhämtning genom stresshantering och mindfulness. 
          </p>
          <p className="about-text">
            I mina behandlingar använder jag ofta klassisk svensk massage i kombination med behandlande och återhämtande spamassagegrepp för att frigöra spänningar och ge djup avslappning. Jag erbjuder även styrke- och balanstester för dig som önskar en positiv livsstilsförändring.
          </p>
          <div className="about-credentials">
            <div className="credential-item">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-award"><circle cx="12" cy="8" r="6"/><path d="M15.477 12.89 17 22l-5-3-5 3 1.523-9.11"/></svg>
              <span>Medlem i Svenska Estetikers Yrkesförbund (SEYF)</span>
            </div>
            <div className="credential-item">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-heart-handshake"><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/><path d="M12 5 9.04 7.96a2.17 2.17 0 0 0 0 3.08v0c.82.82 2.13.85 3 .07l2.07-1.9a2.82 2.82 0 0 1 3.79 0l2.96 2.66"/><path d="m18 15-2-2"/><path d="m15 18-2-2"/></svg>
              <span>Fokus på naturlig återhämtning</span>
            </div>
          </div>
        </div>
      </div>
    </section>
  )
}

export default About
